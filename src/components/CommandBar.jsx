 ...
 */
export function CommandBar({ config, setConfig, sections, loading, error, members, overrideActive, offCount, week }) {
  ...
  const coverageLabel = week.weekStart != null
    ...
    <div className="card commandbar">
      ...
      <div className="budgetBar">
        <div className="budgetField">
          ...
          <input type="range" min={WORD_BUDGET.MIN} max={WORD_BUDGET.MAX} step={WORD_BUDGET.STEP}
      ...
      <div className="row cbPills">
        <span className="pill">This week's readers: <b>{readerNames}</b></span>
      ...
      {error && <div className="err">Couldn't load tafsir: {error}. Check your connection (first load needs internet; it's cached afterwards).</div>}
    </div>
  );
}
